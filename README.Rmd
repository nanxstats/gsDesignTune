...
sequential designs created by `gsDesign::gsDesign()`, `gsDesign::gsSurv()`, and
...
For calendar-timed analyses, use `gsSurvCalendarTune()` with `calendarTime` and
optionally tune `spending = tune_choice("information", "calendar")`.
